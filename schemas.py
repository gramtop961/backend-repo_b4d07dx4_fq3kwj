...
# Example schemas (you can keep these for reference or extend them):
...
# Waffle shop specific schemas

class Waffle(BaseModel):
    """
    Waffle products
    Collection name: "waffle"
    """
    name: str = Field(..., description="Waffle name")
    description: Optional[str] = Field(None, description="Short description")
    price: float = Field(..., ge=0, description="Price in USD")
    image: Optional[str] = Field(None, description="Image URL")
    in_stock: bool = Field(True, description="Availability")
    tags: Optional[List[str]] = Field(default=None, description="Flavor tags")

class OrderItem(BaseModel):
    waffle_id: str = Field(..., description="ID of the waffle product")
    name: str = Field(..., description="Waffle name at time of order")
...
class Order(BaseModel):
    ...
    total: float = Field(..., ge=0, description="Total amount charged")
